...
This mirrors the Gemini pipeline's flow exactly (same schema logic, same
caching, same combined + visual report outputs) -- only the model backend
differs:
  - RESULTS ARE CACHED per file (raw_extraction.json) -- re-running the
    script never re-calls the (slow, local) model on a file that's already
    been extracted. Only the Excel layout gets rebuilt every run, so
    newly-seen extra fields from later files still show up as columns in
    earlier files' sheets too.
...
# Most documents finish comfortably within a normal context window --
# paying for the larger window on every single call slows every file down
# for the sake of the rare ones that need it. So: try fast/small first, and
# only escalate to the bigger window on a retry, and only if the previous
# attempt actually showed signs of running out of room (empty response,
# done_reason == "length", or a truncated/unparseable JSON tail).
...
def extract(path, images, debug_dir=None):
    ...
    # Ran out of room last attempt? -- bump the context window up for the
    # next one. Stays False (fast path) unless we actually see evidence of
    # truncation, so most files run at the cheaper NUM_CTX_DEFAULT speed.
    needs_bigger_ctx = False

    for attempt in range(1, MAX_RETRIES + 1):
        num_ctx = NUM_CTX_RETRY if needs_bigger_ctx else NUM_CTX_DEFAULT
        try:
            resp = requests.post(
                ...
                json={
                    ...
                    "images": images_b64,
                    "stream": False,
                    # qwen3-vl is a reasoning-tuned model that can burn its
                    # entire token budget on internal <think> content and
                    # never emit the actual answer, especially with a long
                    # prompt + OCR hint + image tokens all competing for the
                    # same context window. think=False disables that (only
                    # has effect on models that support the switch; ignored
                    # otherwise).
                    "think": False,
            ...
            # Anything that looks like a truncation (empty output, or the
            # model explicitly reporting it hit the length limit) means the
            # NEXT attempt should use the bigger window, not this one.
            if not raw or done_reason == "length":
                needs_bigger_ctx = True

            if not raw:
                # Empty "response" with no parse-able text at all -- this is
                # NOT a JSON-formatting problem, something upstream failed
                # or the model ran out of budget before producing any
                # output. Dump the FULL payload (not just the empty text
                # field) so the actual cause (done_reason, eval_count, an
                # "error" key, etc.) is visible instead of guessing blind.
                print(f"    [empty response, retry {attempt}/{MAX_RETRIES}] "
                      f"num_ctx={num_ctx} done_reason={done_reason or 'unknown'}"
                      + (" -- retrying with a larger context window"
                         if done_reason == "length" else ""))
                if debug_dir:
                    os.makedirs(debug_dir, exist_ok=True)
                    dump_path = os.path.join(debug_dir, f"raw_response_attempt{attempt}.json")
                    with open(dump_path, "w", encoding="utf-8") as fh:
                        json.dump(payload, fh, ensure_ascii=False, indent=2)
                    print(f"        full API payload saved to {dump_path}")
            else:
...
def write_visual_report(files, per_file_rows, out_path):
    ...
    not the global batch-wide column set. So a cheque's table and a GST
    invoice's table can have completely different columns, which is the
    point: nothing gets forced into a shared shape here."""
    wb = Workbook()
    ws = wb.active
    ws.title = "Invoices"
    ...
    try:
        for f in files:
            ...
            # One sub-table per record (a file can contain multiple cheques/
            # receipts) -- each record gets its own header row since two
            # records even within the same file can carry different fields.
            for rec in rows:
...
def main():
    ...
    print(f"  NUM_CTX             = {NUM_CTX_DEFAULT} (escalates to {NUM_CTX_RETRY} on retry if truncated)")
